...
export class ReportsService {
  ...
  async range(restaurantId: string, from: string, to: string) {
    const restaurant = await this.restaurants.findById(restaurantId);
    const totals = await this.db.one<{
      orderCount: number;
      grossMinor: number;
      ...
      `select count(*)::int as "orderCount",
              ...
              coalesce(sum(tax_minor), 0)::int as "taxMinor"
       from orders
       where restaurant_id = $1 and business_date between $2::date and $3::date
    ...
    const topItems = await this.db.many<{ name: string; quantity: number; amountMinor: number }>(
      `select oi.name_snapshot as name,
       ...
       from order_items oi
       join orders o on o.id = oi.order_id
       where oi.restaurant_id = $1 and o.business_date between $2::date and $3::date
    ...
    const byDay = await this.db.many<{
      businessDate: string;
      orderCount: number;
      grossMinor: number;
    }>(
