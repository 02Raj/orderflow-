      ...
      <div className="grid gap-8 p-5 md:grid-cols-[1fr_320px] md:p-8">
        <div>
          <PageHeader
            kicker="Menu"
            title="Menu"
            copy="Paste the list you already use. Category on its own line with a colon, then each dish and price."
          />
          {setup && categories.length === 0 ? (
            <div className="mt-4 rounded-xl border border-[#ead7a4] bg-[#fff4d6] px-4 py-3 text-sm">
              <strong>Step 1 of 3.</strong> Import your menu, then print table QR codes.
            </div>
          ) : null}
                      ...
                      .map((item) => (
                        <li key={item.id} className="flex items-start justify-between gap-3 py-3">
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            ...
                            <button className="text-xs font-medium text-[var(--chili)]" onClick={() => toggle(item)}>
                              {item.isAvailable ? "Mark sold out" : "Available"}
                            </button>
                          </div>
                        </li>
                      ))}
