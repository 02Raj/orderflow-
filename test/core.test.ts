import { parseMenuText } from '../src/server/menu/menu.service';
import { computeTotals } from '../src/server/common/money';
import { businessDateFor } from '../src/server/orders/orders.service';
...
test('NYC vs Texas sales tax is exclusive and different', async () => {
  const { resolveRegion } = await import('../src/server/auth/tax-regions');
...
test('global country presets include major markets', async () => {
  const { COUNTRY_PRESETS } = await import('../src/server/auth/country-presets');
